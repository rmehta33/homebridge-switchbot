...
* Add Plug Mini (EU) support ([#1344](https://github.com/OpenWonderLabs/homebridge-switchbot/issues/1344)) ([259dbc3](https://github.com/OpenWonderLabs/homebridge-switchbot/commit/259dbc3d9986683934112739416c73daadb7a28f))
...
### ⚠️ Breaking Changes
- Upgrade `node-switchbot` dependency to `^4.0.0` with improved resilience and connection intelligence.
- Matter support: The plugin now supports **Matter child bridge** platform mode for Matter-compatible smart home ecosystems.
  - New `SwitchBotMatterPlatform` platform type for Matter protocol support.
  - Matter devices are registered via the Discovery + Configuration merge model: discovered devices are supplemented by per-device config overrides.
  - Stale accessories (previously registered but no longer discovered/configured) are automatically removed by default. Use `keepStaleAccessories` option to preserve them.
- Device discovery and registration behavior has changed:
  - Matter platform defaults to discovery-first registration with optional config-only device support via `allowConfigOnlyDevices`.
  - Per-device configuration is now merged with discovered device properties (by `deviceId`) instead of being stored separately.
...
- Add unit tests for per-device merging, config-only behavior, BLE parsing, stale-accessory behavior, and mapping helpers, and lifecycle cleanup.
